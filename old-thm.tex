...
the induced connecting metric $|x-y|_f$.
...
Our first formulation use $\langle|{*}-{*}|\rangle$-metric instead of $\langle{*}-{*}\rangle$ which used in the old formulation.
...
From the main theorem in \cite{petrunin-stadler} it follows that $\mu$ is monotonic and therefore an isometry --- a contradiction.
\qeds

The second formulation use the metric $\langle{*}-{*}\rangle$ as in the original formulation.
...
Indeed, once the claim is proved, one can apply it recursively for a sequence of $\eps_n$ that converge to zero very fast.
...
Note that there is a simple curve from $x$ to $y$ that is runs in the constructed arcs and $\Phi$.
...
Note that the graph is connected therefore we may choose a path from $\alpha_0$ to $\alpha_n$.
