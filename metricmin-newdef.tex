...
%(If the map $s$ is a Lipschitz embedding, then the map $\hat\pi_s\:\DD\to \<\DD\>_s$ is a Lipschitz homeomorphism.
%In this case any majorization $\mu$, the map $s''=\hat s'\circ \mu\circ \hat\pi_s\:\DD\to Y$ satisfies the inequality $\length s\circ\gamma\ge \length s''\circ\gamma$ for any curve $\gamma$ in $\DD$; moreover, if $\mu$ is not an isometry, then the inequality is strict for some curve. 
%)
...
The space $\<|X|_f\>_{\bar f}$ is the intrinsic metric on $|X|_f$, which is $\<|X|_f\>$ in the notation defined below.
...
\bibitem{St} Stadler, S. \textit{The structure of minimal surfaces in CAT(0) spaces}, \texttt{arXiv:1808.06410 [math.DG]}.
