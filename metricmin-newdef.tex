...
and $\gamma\:\SS^1\to Y$ be a closed rectifiable curve.
...
This statement generalize the main theorem in \cite{petrunin-metric-min}; we also fill a gap in the proof.
...
In section \ref{sec:smooth} we discuss the relation between saddle surfaces and metric minimizing discs.
Evidently smooth metric minimizing disc $s$ in a Euclidean space is saddle, we prove 
a local converse of this statement in dimension three and show by example that there is no global converse.
...
An old conjecture of Samuel Shefel states that any saddle disc in a $\RR^3$ is $\CAT[0]$ with respect to its intrinsic metric.
