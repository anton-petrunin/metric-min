...
We say $s$ is \emph{metric minimizing} if one cannot shrink its intrinsic metric. Explicitly, this means that a map $s'\:\DD\to \RR^3$ which agrees with 
...
This is a fundamental result which subsumes and generalizes many known results.
...
The ruled discs considered by Alexandrov are evidently metric minimizing. Harmonic maps in the sense of Korevaar-Schoen \cite{KS} from the disc to $\CAT(0)$ spaces are metric minimizing.
...
see also \cite[Chapter 4]{akp} and \cite{petrunin-stadler}. Our result is new, even for harmonic discs.
...
Our main theorem already has many applications and we expect many more to come.
...
\parit{Proof.}
Since $\bar f$ is short, we have $\length(\bar f\circ\gamma)\leq \length(\gamma)$ for every curve $\gamma:[0,1]\to|X|_f$.
