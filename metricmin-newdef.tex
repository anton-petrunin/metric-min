...
that is, for any hyperplane $\Pi$ each of the connected components of $\DD\backslash s^{-1}\Pi$ meets the boundary.
In section \ref{sec:smooth} we prove a converse for the local statement in dimension three and show by example that there is no global converse.
We also conjecture that in the 4-dimensional case even the local converse does not hold.
...
\section{Smooth surfaces}\label{sec:smooth}
 ...
 It is an old conjecture of Shefel that any saddle disc in a $\CAT[0]$ space is
locally $\CAT[0]$ with respect to its intrinsic metric. See \cite{petrunin-metric-min}
for the definition of a general saddle disc and a precise formulation of the conjecture.
In this section we will discuss the relation between metric minimizing discs and saddle discs in
low-dimensional Euclidean spaces. 

\parbf{Three-dimesional case.}
In general a smooth saddle disc may not be globally metric minimizing.
...
We do not see any way to show that a given surface is metric minimizing except by constructing an appropriate energy 
for which it is energy minimizing, as we did in Section~\ref{sec:smooth}.
Locally, the energy functional as in Section~\ref{sec:smooth} can be described by three functions defined on the disc.
