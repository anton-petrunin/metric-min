...
The corresponding metric space will be denoted by $\<\DD\>_s$;
...
The map $s\:\DD\to Y$ is called \emph{metric minimizing disc} if $s\succcurlyeq s'$ implies that the corresponding majorization $\mu$ is an isometry.
...
If the condition that the boundary curve is rectifiable is removed, the space $\<\DD\>_s$ might have points on infinite distance from each other, but as our proof shows all the triangles with finite sides in $\<\DD\>_s$ are thin; in particular each metric component of $\<\DD\>_s$ is a $\CAT[0]$ space.
In fact one can consider $\<\DD\>_s$ as a $\CAT[0]$ space if infinte distances beteen points are legal.

It is evident that any metric minimizing disc $s$ the Euclidean space is saddle; 
that is for for any hyperplane $\Pi$ each of the connected components of $\DD\backslash s^{-1}\Pi$ meets the boundary.
In section \ref{sec:smooth} we prove partial converse in 3-dimensional case, see also final remarks.
...
However, this is the case for metric minimizing discs $f\:\DD\to Y$; see
