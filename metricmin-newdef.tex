...
\end{itemize}
If in addition $|x-y|=0$ implies $x=y$ then the pseudometric $|{*}-{*}|$ is called a \emph{metric space}; some authors prefer to call it \emph{$\infty$-metric space} to emphasize that the distance between points might be infinite.
...
Let us define the \emph{length pseudometric} on $X$ induced by $f$;
it is defined as
\[\< x-y\>_f
=
\inf
...
If $X$ is a metric space, the constructions above can be applied for the identity map $\id\:X\to X$.
...
Note that $|X|$ is a locally connected and the tautological map $|X|\to X$ is short and length-preserving.
...
Note that $\iota_f\: \hat x\to \bar x$ defines a map $\iota_f\: \<X\>_f\to |X|_f$ and by construction it is length-preserving.
Since $\<X\>_f$ is a length space, the latter implies that $\iota_f$ short.
In general $\iota_f$ might be not injective, an example is given in \cite[4.2]{petrunin-intrinisic}.
Below (Proposition~\ref{prop:|D|}) you will see that for metric minimizing disc $f\:\DD\to Y$ the map $\iota_f$ is injective.
