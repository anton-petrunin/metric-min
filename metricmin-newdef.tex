...
\parbf{Motivation and background.}

Let us denote by $\DD$ the closed unit disc in the plane.
Its boundary $\partial \DD=\SS^1$ is the unit circle.
Let $X$ be a $\CAT(0)$ space and $\gamma:\SS^1\to X$ an embedded rectifiable loop. A ruled surface bounded by $\gamma$
is an extension $f:\DD\to X$ of $\gamma$ obtained by choosing a basepoint $o$ on $\SS^1$ and sending each chord with 
initial point $o$ to the corresponding geodesic. The image of $f$ is the geodesic 
cone over $\gamma$ with respect to $o$. Alexandr Alexandrov  proved in \cite{A} that such a ruled surface is 
a $\CAT(0)$ disc with respect to its intrinsic metric. This remarkable result reduces certain questions about 
general $\CAT(0)$ spaces to the much more feasible case of surfaces. For instance, Alexandrov deduced the Euclidean
quadratic isoperimetric inquality for curves, meaning that any rectifiable loop of length $l$ in a $\CAT(0)$ space
bounds a disc of area at most $\frac{l^2}{4\pi}$. 

Very recently Alexander Lytchak and Stefan Wenger \cite{LW5} obtained a similar 
result about the intrinisic geometry of minimal discs in $\CAT(0)$ spaces. This led to a new method for constructing nonpositively curved spaces.
According to \cite{LS}, each $\CAT(0)$ space has a vast number of nontrivial deformations.

The goal of this paper is to prove an analogous structural result for a much more general class of maps. These so called {\em metric minimizing maps}
provide a flexible new tool to study the geometry and topology of nonpositively curved spaces.
...
We say $s$ is \emph{metric minimizing} if its intrinsic metric is minimal. Explicitly, this means that  a map $s'\:\DD\to \RR^3$ which agrees with 
$s$ on $\partial \DD$ can only fulfill 
\[\length s\circ\gamma\ge \length s'\circ\gamma\]
for all curves $\gamma$ in $\DD$, if equality holds for all $\gamma$.

As it follows from the main theorem below, the induced metric on the disc for a metric minimizing embedding is $\CAT(0)$.

To formulate the theorem in full generality we need to extend the definition of metric minimizing maps in two ways. First, we will only assume continuity of $s$, so the map $s$ might not be an embedding;
this part is tricky.
...
if it is an extension,  $s|_{\SS^1}=\gamma$.
...
Observe that this notion is invariant under reparametrizations. If $s\:\DD\to Y$ is metric minimizing
and $\varphi:\DD\to\DD$ is a homeomorphism, then again $s\circ\varphi$ is a metric minimizing disc.
...
Note that a disc retract (for example the one shown in the picture) may have essentially different boundary curves,
...
The class of metric minimizing discs is huge --- if the ambient space $Y$ is $\CAT(0)$, then one can use ultra-limits to find a metric minimizing  disc \emph{below} 
any map $\DD\to Y$ with respect to $\succcurlyeq$. 
More importantly, the class of metric minimizing discs contain most, if not all well-studied disc-type maps in metric geometry.
...
Indeed, harmonic discs are solutions to the Dirichlet problem, i.e. energy minimizing fillings of given loops.
...
As a consequence, minimal discs in the sense of Lytchak-Wenger \cite{LW} are metric minimizing. These are Douglas-Rado solutions to the Plateau problem and therefore harmonic. 
...
Although some special cases of Theorem \ref{thm:main} were known, the result is new even for harmonic discs.
...
The curvature of metric minimizing discs has already had several applications and we expect many more to come.
The intrinsic geometry of metric minimizing surfaces was 
used in \cite{L} to study sets of positive reach (ruled surfaces); in \cite{AB} to generalize the Gauss equation to non-smooth spaces (ruled surfaces);
in \cite{G} to bound the complexity of smooth maps (general metric minimizing surfaces); in \cite{LS} to deform general $\CAT(0)$ spaces (minimal discs) and in 
\cite{St} to control the mapping behaviour of minimal surfaces (minimal discs).  
...
is connected. Hence 
\[\length(\gamma)\leq \sum_i \diam f(\bar\pi_f^{-1}(\gamma_i))+\eps=\sum_i \diam(\bar f(\gamma_i))+\eps\leq \length(\bar f\circ\gamma)+\eps.\]
...
By smoothing this example one can produce a smooth saddle disc which is not metric minimizing.
...
\bibitem{L} Lytchak, A,\textit{On the geometry of subsets of positive reach.} Manuscripta Math., Vol.115 (2004), 199-205. 


\bibitem{LS} Lytchak, A.; Stadler, S.,  \textit{Conformal deformations of CAT(0) spaces}, preprint arXiv:1711.04507, 2017.
...
\bibitem{St} Stadler, S. \textit{On the theorem of Fary-Milnor}, in preparation.
