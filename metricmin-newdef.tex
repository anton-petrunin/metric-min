...
We say $s$ is \emph{metric minimizing} if one cannot shrink its intrinsic metric. Explicitly,  a map $s'\:\DD\to \RR^3$ which agrees with 
$s$ on $\partial \DD$ can only fulfill 
\[\length s\circ\gamma\ge \length s'\circ\gamma\]
for all curves $\gamma$ in $\DD$, if equality holds for any $\gamma$.
...
The ruled discs considered by Alexandrov \cite{A} are evidently metric minimizing. Harmonic maps in the sense of Korevaar-Schoen \cite{KS} from the disc to $\CAT(0)$ spaces are metric minimizing.
In particular, minimal discs in the sense of Lytchak-Wenger \cite{LW} are metric minimizing. These are Douglas-Rado solutions to the Plateau problem and therefore harmonic. 
...
The main theorem subsumes and generalizes several previously known results, including a theorem of Alexandr Alexandrov about ruled surfaces in \cite{A} and 
...
It also generalizes a result on minimal surfaces by Alexander Lytchak and Stefan Wenger 
...
Our main theorem has already had several applications and we expect many more to come.
The intrinsic geometry of metric minimizing surfaces was 
used in \cite{L} to study sets of positive reach; in \cite{AB} to generalize the Gauss equation to non-smooth spaces;
in \cite{G} to bound the complexity of smooth maps; in \cite{LS} to deform general $\CAT(0)$ spaces and in 
\cite{St} to control the mapping behaviour of minimal surfaces.  
...
Assume that each integral curves for one of vector fields $v_i$ goes from boundary to boundary and sweeps this way whole $\DD$. 
...
Since $\DD$ is sweeped out by arcs of integral curves of $v_i$, the latter implies that $s_0=s_1$.
...
\bibitem{G} Gromov, M., \textit{Singularities, expanders and topology of maps. Part 1 : Homology versus volume in the spaces of cycles.} Geom. func. anal.,19 (2009) n 3, 743-841.
