...
Further the maps satifying the assumptions in the proposition will be called \emph{cell-like}.%???IS IT OK TO CALL IT THIS WAY???
...
and $\eps>0$,.
Assume $s\:\DD\to W$ is a saddle map and $u\:\partial\DD\to K$ is a continupus map such that 
\[|s(x)-u(x)|<\eps\]
for any $x\in\partial \DD$.
Then $u$ can be extended to a continuous map whole $\DD$ such that inequality 
\[|s(x)-u(x)|<\eps\]
holds for any $x\in\DD$.
\end{thm}

\begin{thm}{Lemma}
Let $W$ be a metric space.
Given a continuous map $s\:\DD\to W$
there is a locally cell-like map $s'\:\DD\to W$
...
Note that the that image of each triangle $\triangle$
...
In other words, the space $\|\DD\|_u$ can be reconstructed by gluing solid geodesic triangles in $W$ along their common sides.
...
and can be constructed the following way:
take at most $N$ convex sets in $W$ 
glue them together along arcs in their boundaries.

Note that $\|\DD\|_u$ belongs to $\mathcal{S}_N$ and $\mathcal{S}_N$ is compact in Gromov--Hausdorff topology.


Assume $\gamma$
is a geodesic $\gamma$ in $W$
such that 
the complement $\DD\backslash u^{-1}(\gamma)$ contains a connected component $\Omega$ without boundary points of $\DD$.
Let us redefine the map $u$ in $\Omega$.
More precisely, let us construct a new map $w\:\DD\to W$
such that $w(x)=u(x)$ for $x\notin\Omega$.

To do this first fix a continuous function $\lambda\:\Omega\to [0,1]$
...
\begin{nnthm}{Claim}
For some choice of $\lambda$ the map $v\:\DD\to W$ satisfies
\[|v(x)-s(x)|<\eps\] 
for any $x$.
\end{nnthm}
...
Now if for some point $p\in  W$ the complement inverse image $\DD\backslash v^{-1}\{p\}$ has a connected component $\Omega$ without points on the boundary reset $v(x)=p$ for any $x\in\Omega$.
Repeating this procedure we obtain a new map, say $w\:\DD\to W$,
such that for any $p\in  W$ any connected component of the complement of the inverse image $\DD\backslash v^{-1}\{p\}$ has  points on the boundary. Note that we still have 
\[|w(x)-s(x)|<\eps\]
for any $x\in\DD$.

Note that by construction, $\|\DD\|_w$ belongs to $\mathcal{S}_N$.
So we can apply the construction $w$ form $u$ recursively.

We can assume in addition that for any point $p\in W$
each connected component of the complement $\DD\backslash u^{-1}\{p\}$ has points in the boundaray $\partial\DD$.
If this is not the case redefine the map $u$ as $\bar u(x)=p$  for any point $x$ inside the violating component 
and $\bar u(x)=u(x)$ otherwise.
Since the map is saddle, the inequality \ref{eq:|s-u|<e} will still hold.
...
from $[x'y']$.
\end{thm}
\parit{Proof of Lemma\ref{lem:length continuity}.}
