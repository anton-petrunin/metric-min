...
We follow the rough outline of the proof of Shefel in \cite{shefel-2D}.
However, Shefel's proof was written rather tight and generalizing from the plane to a
disc retract come with some additional complications, so we have decided to provide all details.
...
Choose a point $z$ on the segment $p\bar p$ such that the angle between $zp$ and $z\bar p$ is less than $\frac{5\pi}{4}$.
Since $z$ is not a cut point, the link $\Sigma_z W$ is connected. Hence there is a direction $v\in \Sigma_z W$
which encloses an angle of $\frac{\pi}{2}$ with the geodesic $z\bar p$. Let $(v_k)$ be a sequence of directions with angles
between $\frac{\pi}{4}$ and $\frac{\pi}{2}$, converging to
$v$ and such that there are geodesics $\gamma_k\in\Geod(W)$ with $\dot\gamma_k(0)=v_k$. If some $\gamma_k$
is disjoint from $C$, then, by the assumptions on the angles, it is also disjoint form $p$ and we are done. Else, all
geodesics $\gamma_k$ intersect $C$. Necessarily, the intersects have to occur for positive times. Therefore, the $\gamma_k$ have uniform length
...
\begin{thm}{Lemma}\label{lem:puzzle-compact}
 We $W$ be a CAT[0] disc retract and fix a number $N$. Consider the class of metric spaces $\mathcal{S}_N$ 
which are homeomorphic to a disc retract
and can be covered by at most $N$ closed convex subsets of $W$.
...
Indeed, fix a triangle $\triangle$ in $\|\DD\|_s$. HOW IS IT GEODESIC?
...
where the intersection is taken over all finite subsets $F$ in $\DD$. ALL? OR NESTED SEQUENCE EXHAUSTING A DENSE SET  

Fix a space $W$ from $\mathcal{W}$
and a dense sequence of points $\{x_1,x_2,\dots\}$ in $\DD$, OR $\|\DD\|_s$ ???
...
such that 
\[q\circ p|_{\partial\DD}=s|_{\partial\DD}.\] HOW DID WE GET q???
