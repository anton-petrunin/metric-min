...
a recent result by Lytchak and Wenger from area minimizers to energy minimizers.%???ref???
...
Indeed our result combined with the smoothability of $\CAT[0]$-surfaces %???ref???
...
In section 7 we treat the two-dimensional case, i.e. metric minimizing discs
in $\CAT[0]$-surfaces and generalize Shefel's result on saddle surfaces. 
...
Denote $Z_f:=X/\sim$ the quotient metric space. 
...
Let $\gamma_0,\gamma_1\:\II\to X$ be two rectifiable paths parametrized by arc length;
denote by $\gamma_t\:\II\to X$ their geodesic homotopy. 
If 
...
Convexity of the metric implies $|\dot\gamma_{\frac12}(t)|\leq 1$. 
By assumtion, the length of $\gamma_{\frac12}$ equals $L$, hence $|\dot\gamma_{\frac12}(t)|=1$ for almost all $t\in[0,L]$. 
So $\gamma_{\frac12}$
is parametrized by arc length. 

Since the energy of a unit speed path equals its length, we conclude from energy convexity 
$$
\int\limits_0^L|\nabla |\gamma_0-\gamma_1||^2 d\mathcal{H}^1
=
0.
$$
Therefore, $\gamma_0$ is parallel to $\gamma_1$.
\qeds

\parbf{Remark.}
It is possible to show in the above situation that  $h([0,1]\times[0,L])$ is intrinsically flat.

\begin{thm}{Corollary}\label{cor:parpaths}
Let $\gamma_0,\gamma_1\:[0,1]\to X$ be two rectifiable paths both starting in a point $p\in X$ and of equal length $L>0$. 
If the lengths of the curves $\gamma_t$ is constant, then $\gamma_{0}=\gamma_{1}$.
\end{thm}
...
\parit{Proof.}
Let $\mathcal{O}\subset\mathcal{S}(f)$ be a well-ordered subset. 
If $\mathcal{O}$ does not have a minimum, then we define
$$
...
\parbf{Sobolev spaces and energy.}
...
\[E(u)=
\int\limits_{\DD^\circ}\left(\frac{1}{\pi}\cdot\int_{\mathbb{S}^1}|(d_zu)(v)|^2\cdot d_v\length\right)\cdot d_z\area\]
is finite. 
Here $du_z$ denotes the approximate metric differential of $u$ at $z$. (See \cite{LW}.)
...
Let $u\:\DD^\circ\to X$ be a continuous map. Then $u$ has finite energy iff
...
Let $u\:\DD^\circ\to X$ be a Sobolev disc. Let $z_0\in\DD$ and $\delta\in(0,1)$. For each $r\in(0,1)$
let $\gamma_r$ be an arc-length parametrization of $\{z\in \DD^\circ:\ |z-z_0|=r\}$. Then there exists
...
\parbf{Harmonic discs are metric minimizing.}
Our plane is to prove that harmonic maps from discs to $\CAT[0]$ spaces are metric minimizing ( Proposition~\ref{prop:harmonic}).
...
$u$ is saddle. %??? something wrong here --- we refer to the theorem below, and also a short map (formally) might preserve the energy???
...
$$
\length(s\circ\pi_u\circ \gamma)=\int_\II |d(s\circ\pi_u)_{\gamma(t)}(\dot\gamma(t))|dt\leq \int_\II |du_{\gamma(t)}(\dot\gamma(t))|dt=\length(u\circ \gamma).
$$
...
\parbf{Intrinisic structure and regularity of harmonic discs.}                                                     
