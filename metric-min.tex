...
Moreover, if $s|_{\partial \DD}$ is rectifiable, then $\|\DD\|_s$ is a $\CAT[0]$ space. 
\end{thm}
...
Given a finite set $F\subset \DD$ which maps to a single metric component of $\|\DD\|_s$,
...
Note that for two such finite sets $F\subset F'$ in $\DD$,
...
\parit{Proof.}
The regularity statement in Theorem \ref{thm:dirichlet} allows us to apply Proposition \ref{prop:exist} to obtain a metric minimizing
disc $s$ with $u \succcurlyeq s$ and $u|_{\partial\DD}=s|_{\partial\DD}$. Since $u$ is continuous, it has finite Korevaar-Schoen energy iff
