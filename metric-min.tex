...
and $u\:\DD\to X$ an energy minimizing disc with fixed rectifiable boundary curve $\gamma$.
Then $u$ is metric minimizing relative $\gamma$.
\end{thm}


\parit{Proof.}
We will first prove the existence of a metric minimizing
disc $s$ with $u \succcurlyeq s$ and $u|_{\partial\DD}=s|_{\partial\DD}$. 
Define the relative sublevel $\mathcal{F}(u)$ as the family of all saddle maps $f:\DD\to X$
with $f|_{\partial \DD}=u|_{\partial \DD}$ and $u\succcurlyeq f$.
We claim that $\mathcal{F}(u)$ is equicontinuous. This is clear for points in the interior of $\DD$. So let 
...
order $``\succcurlyeq ''$ on $\mathcal{C}(D,X)$.
...
\begin{thm}{Proposition}\label{prop:exist}
 ...
 there is an intrinsically continuous  metric minimizing map $s\:\DD\rightarrow X$
 such that $\|{*}-{*}\|_s\leq\|{*}-{*}\|_f$.
\end{thm}
