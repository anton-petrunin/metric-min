...
and extend it to the whole disc by using barycentric maps on simplices.
...
Moreover,  $\|\triangle\|_u$ is isometric to the image $u(\triangle)$ because $u$ embeds every
simplex {\bf ??}.

It follows that the space $\|\DD\|_u$ can be reconstructed by gluing solid geodesic triangles in $W$ along convex subsets containing common sides.
...
along SOMETHING.
Note that $\mathcal{S}_N$ is compact in Gromov--Hausdorff topology (BY COMPACTNESS LEMMA?).
