...
A continous map $s\:\DD\to W$ is called \emph{saddle} if for any closed convex subset $K\subset W$ 
...
$x\in \Omega$, set $v(x)$ to be the point which minimizes 
\[\lambda\cdot\dist^2_p+(1-\lambda)\cdot\dist^2_q,\]
where $p$ and $q$ are correspondingly the closest point projections of  $u(x)$ and $s(x)$ onto $\Conv u(\partial\Omega)$.
...
we only need to apply the lemma for the convex set $K'=\Conv u(\partial\Omega)$.

To prove \ref{lem:cutting-hat:b}, we need to remove bubbles form the constructed map $v$.
Namely for any $x$ we can choose the maximal open set $\Upsilon_x$ such that for some point $p\in W$ the complement $\DD\backslash v^{-1}\{p\}$ has $\Upsilon_x$ as a
...
and extend it to whole $\DD$ by using barycentric map on each simplex.
...
Note that the space $\Gamma$ of all geodesics in $W$ with the ends on $\partial W$ is compact. 
