...
seperates $x$ and $y$, then they are called {\em conjugate}%???
. 

For a non-cut point $p\in X$
we define $conj(p)$ as the set of all points conjugate to $p$. A point $e\in X$ is called an {\em endpoint},
if $conj(e)=\{e\}$. 
The conjugacy set $conj(p)$ for non-endpoints
are called {\em cyclic elements}. %??? STRANGE DEF, maybe set of cyclic elements, or  support of cycles
...
connected and locally connected metric space.}, then every cyclic element $conj(p)$ is {\em cyclically connected}; that is,
...
$J_n'\cap J_n$ contains at least two points. Consequently $J_n'$ lies in the same cyclic element as $J_n$; that is, $J_n'\subset C_n$. As
...
Let us remove bubbles from $f_\infty$, denote the obtained map by $f_\infty'$.
...
the space $\Gamma$ of geodesic segments with endpoints on $\partial W$ is compact as well.

\begin{thm}{Lemma}\label{thm:saddle-2D}
Let $W$ be a $\CAT[0]$ space which is a disc retract. Furthermore, let $\mathcal{G}$ be a countable dense subset 
of $\Gamma$.
If $s\:\DD\to W$ is a map such that for any element $g$ in $\mathcal{G}$ every 
component of $\DD\backslash s^{-1}(g)$ intersects the boundary $\partial\DD$,
then $s$ is saddle.
\end{thm}

\parit{Proof.} Assume that there is a closed convex set $K$ in $W$ such that 
$\DD\backslash s^{-1}(g)$ has a component $U$ which is disjoint from the boundary $\partial\DD$. Note that 
$U$ is open. Pick a point $p$ in $U$. Denote $\overline{s(p)}$ the closest point projection of $s(p)$ onto $K$.
It is enough to find an element in $\mathcal{G}$ which seperates $s(p)$ and $\overline{s(p)}$.
If the geodesic segment $s(p)\overline{s(p)}$ contains an interior point $x$ where the link $\Sigma_x W$ consists of 
more than two points, then we are done. On the other hand, if we do not find such a point then any point on $s(p)\overline{s(p)}$
is a cut point, in particular $s(p)\overline{s(p)}$ is contained in $\partial W$. 
Since $K$ intersects the image of $s$ the segment $s(p)\overline{s(p)}$ has to lie entirely in the image of $s$.
But then a subinterval of $s(p)\overline{s(p)}$ seperates $s(p)$ and $\overline{s(p)}$ and by density we can find an element in $\mathcal{G}$ with the same property.
that its inverse image separates $p$ from the boundary $\partial \DD$.
...
parametrizations on each side, then there is an extension $F$ of $f$ to a map between convex hulls
$F:ch(\Delta)\to ch(\Delta')$ such that $F$ induces an isometry $\bar F:\|ch(\Delta)\|_F\to ch(\Delta')$. 
\end{thm}

\parit{Proof.}
There are obious cases, namely if $\Delta'$ is either embedded or contained in a single geodesic. 
...
a nondegenerated vertex, in the sense that the image of adjacent sides only intersect in the image of the vertex. Repeating at most two times reduces the general case to the above obvious ones.
\qeds
...
Note that $\mathcal{S}_N$ is compact in Gromov--Hausdorff topology (BY MODIFIED COMPACTNESS LEMMA?).

From above, $\|\DD\|_u$ belongs to $\mathcal{S}_N$.  

Note that the space $\Gamma$ of all geodesics in $W$ with endpoints on $\partial W$ is compact. 
...
Denote by $w_n\:\|\DD\|_{u_n}\to W$ the projection.

Since $\mathcal{S}_N$ is compact, we can pass to its partial limit of $\|\DD\|_{u_n}$ as $n\to\infty$,
say $Q_\eps$.
...
Indeed, there should be a point, say $y$ in one of the convex sets in the decomposition of $Q_\eps$ which disappears after cutting along $\gamma$.
