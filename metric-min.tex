...
\footnote{To remove possible psychological barriere with infinity,
let us mention an other construction  --- it will not be used further.
Consider an equivalence relation ``$\approx$'' on a metric space defined as \[x\approx y\  \iff\  |x-y|<\infty.\]
Note that its equivalence classes form genuine metric spaces; 
that is, in addition to the conditions above we have $|x-y|<\infty$ for any pair of points $x$ and $y$.
It implies that metric spaces in our definition 
can be thought of disjoint union of some collection genuine metric spaces with infinite distance from each other.}
...
It remains to note that $X$ is embedded into $\RR^2_f$ \qeds
...
Then $|\DD|_f$ is homeomorphic to a disc retruct;
that is $|\DD|_f$ compact simply connected space which admits an embedding $\iota\:|\DD|_f\to \RR^2$.
...
Let $W$ be a $\CAT[0]$ space which is homeomorphic to a plane continuum.
Assume $s\:\DD\to W$ be a map such that for any geodesic $\gamma$ in $W$ 
any connected component of the complement $\DD\backslash s^{-1}(\gamma)$
has a point from the boundary $\partial \DD$.
Then $\|DD\|_s$ is $\CAT[0]$.
\end{thm}

We repeat word-by-word the proof of Shefel in \cite{shefel-2D}, 
the only difference is instead of ``plane'' one has to say ``$W$''.

\parit{Proof.}
Fix a fine triangulation $\tau$ of $\DD$.
Map the vertices of $\tau$ by $s$
and extend it to whole $\DD$ using barycentric map.

We get a map, say $u\:\DD\to W$.
Note that $u$ can be made arbitrary close to $s$ assuming that the triangulation is fine.

If $s_\tau$ does not satisfy the condition of the theorem,
...
Redefine the map $u$ on $\Omega$ to the closest-point projection to $\gamma$.

Now let us apply the above procedure recursively
for a dense sequence of geodesics $\gamma_1,\gamma_2,\dots$ in $W$.
We obtain a sequence of maps $u_1,u_2,\dots$ which define a short map of $\|D\|_u$.
In particular a subsequecnce of $(u_n)$ converges,
denote its limit as $v$.
(In fact it is true that $(u_n)$ converges, but we do not need it.)
...
and $s\:\DD\to Y$ is a metric minimizing map rel. to the boundary $\partial\DD$.
