...
Let us consider a smooth surface $\Sigma$ immersed in $\mathbb{R}^n$.  
...
But as it turns out, both conditions are irrelevant in order for the intrinsic metric to be nonpositively curved.

The aim of this paper is to prove a global structural result about the intrinsic metric of a much more general class of surfaces.
We will study "metric minimizing" surfaces in general $\CAT[0]$ spaces\footnote{A general $\CAT[0]$ space is not necessarily locally compact.}. Their defining property is that any other parametrized surface with
equal boundary increases the intrinsic distance for at least two points of the domain. Examples of metric minimizing surfaces are
ruled surfaces, energy minimizing and area minimizing surfaces. 


 For the intrinsic structure of metric minimizing surfaces 
we obtain global geometric and topological information. A special case of our main theorem reads
...
This is a substantial extension of the results in \cite{petrunin-metric-min}. It also generalizes Alexandrov's theorem on ruled surfaces.
Another source for applications comes from the following result.
...
A combination of Theorem \ref{thm:mainintro} and Theorem \ref{thm:harmonicintro} generalizes in the $\CAT[0]$-setting
...
and minimal surfaces. Indeed our result combined with the smoothability of $\CAT[0]$-surfaces allows to reduce regularity questions to 
the smooth case. In particular we obtain for energy minimizing discs quick alternative proofs of Korevaar and Schoen's interior regularity
result as well as Serbinowski's boundary regularity result. 
...
use a theorem of Moore \cite{} to obtain topological information about  their 
