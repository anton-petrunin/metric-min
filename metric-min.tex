...
Let $\Omega\subset \mathbb{R}^n$ be an open and bounded subset. Further, let $(X,d)$ be a metric space.
A measurable map $u:\Omega\to X$ lies in 
...
$$
exists, then it is called {\em approximate metric derivative} in the direction $v\in\mathbb{R}$ at the point $z$. It will be denoted
as $|du_z(v)|$. According to
\cite[Prop.4.3]{LW}, if $u\in W^{1,2}(\Omega,X)$, then its approximate metric derivative exists in every direction at almost every point $z\in\Omega$,
and defines a semi-norm $|du_z|$ on $\mathbb{R}^n$. Moreover, the function $z\to\int_{S^1}|du_z(v)|^2 d\mathcal{L}^1$ is in $L^2(\Omega)$.
We define the {\em energy} of a map $u\in W^{1,2}(\Omega,X)$ as
$$
...
We say that two rectifiable paths $\gamma_{\pm 1}:I\to X$ are {\em parallel}, if $d(\gamma_{-1},\gamma_{+1})$
is constant on $I$.

\begin{thm}{Lemma}
Let $\gamma_{-1}$ and $\gamma_{+1}$ be two parallel paths and $h:I^2\to X$ the geodesic homotopy
between. Then $h(I^2)$ is intrinsically flat.
\end{thm}
\parit{Proof.}
...
\begin{thm}{Lemma}
Let $\gamma_{\pm 1}:I\to X$ be two rectifiable paths of equal length $L>0$. Set $\gamma_t:=h(\cdot,t)$ where $h$ denotes the geodesic homotopy
