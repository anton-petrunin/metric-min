...
\begin{thm}{Lemma}\label{lem:harmonic}
Let $X$ be a $\CAT[0]$ space 
and $u\:\DD\to X$ an intrinsically continuous harmonic disc.
Then $u$ is metric minimizing relative $\partial\DD$.
\end{thm}


\parit{Proof.}
By Proposition \ref{prop:exist} there exists an intrinsically continuous metric minimizing map
$s:\DD\to X$ with $u\succcurlyeq s$. Since $u$ is continuous, it has finite Korevaar-Schoen energy iff
...
$$
The Lebesgue Differentiation theorem implies $ds\leq du$ almost everywhere. Since $E$ is monotonic in the metric differentials, it follows that 
...
Let $X$ be a $\CAT[0]$ space 
and $u\:\DD\to X$ a harmonic disc with rectifiable boundary $u|_{\partial \DD}$.
Then $u$ is intrinsically continuous and metric minimizing relative $\partial\DD$.
\end{thm}


\parit{Proof.}
 For $r\leq 1$ set $B_r:=\overline{B_r(0)}\subset\DD$ and $S_r:=\partial B_r$. Choose a sequence $r_k\to 1$. From the regularity part of Theorem \ref{thm:dirichlet}
 we get that $u_k:=u|_{B_{r_k}}$ is a Lipschitz continuous harmonic disc.  
 such that $\length (u(S_{r_k}))$ is finite. By Lemma \ref{lem:harmonic}, Theorem \ref{thm:main} and Corollary \ref{cor:main} the spaces $Z_k:=\|\DD\|_{u_k}$ are $\CAT[0]$ disc retracts. 
 The natural maps $Z_k\hookrightarrow\|\DD\|_u$ are length preserving and the union $\bigcup_k Z_k$ contains the interior of $\|\DD\|_u$. The Courant-Lebesgue Lemma (Lemma 7.3 in \cite{LW})
 implies $\overline{\bigcup_k Z_k}=\|\DD\|_u$. It follows that $\|\DD\|_u$ is a $\CAT[0]$ space. Since $u|_{\partial \DD}$ is rectifiable the same argument as in Corollary \ref{cor:main} shows that 
 $\|\DD\|_u$ is actually a $\CAT[0]$ disc retract.
 Next we prove the intrinsic continuity of $u$. By Theorem \ref{thm:dirichlet} we only need to look at boundary points. Choose $z\in\partial\DD$. From the   
 Courant-Lebesgue Lemma and the rectifiability of $u|_{\partial \DD}$ we get for every $\epsilon>0$ a Jordan curve $J$ in $\DD$ which contains $z$ and bounds a neighborhood $U$ of $z$ 
 such that $\length(u(J))<\epsilon$. Since $\|\DD\|_u$ is a $\CAT[0]$ disc retract, any point in $U$ has intrinsic distance less than $\frac{\epsilon}{\pi}$ from $z$. Hence $u$
 is intrinsically continuous and Lemma \ref{lem:harmonic} implies that $u$ is metric minimizing relative $\partial\DD$.
 \qeds
 
 
 
\parbf{Remark.}
This proposition immediately generalizes to other convex energy functionals which are monotonic in the metric differentials and have locally intrinsically
continuous minimizers.
