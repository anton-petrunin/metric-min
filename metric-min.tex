...
if there is no such set we set $|x-y|_f=\infty$. The associated nondegenerated pseudometric space will be
denoted as $|X|_f$.

The intrinsic metric induced by $|{*}-{*}|_f$ will be indicated by 
$\|{*}-{*}\|_f$. 
That~is, 
...
is named $\|X\|_f$. Note that by definition, each metric component of $\|X\|_f$ is a length space.
...
Then $\llangle\DD\rrangle_s$ is $\CAT[0]$.
\end{thm}
...
$F\:\Conv(\Delta)\to \Conv(\Delta')$ between convex hulls which induces an isometry $\bar F\:\llangle\Conv(\Delta)\rrangle_F\to \Conv(\Delta')$. 
\end{thm}
...
Denote by $\pi\:\DD\to \llangle\DD\rrangle_u$ the natural projection and
by $\bar u\:\llangle\DD\rrangle_u\to X$ the induced map.
...
Moreover,  $\llangle\triangle\rrangle_u$ is isometric to the image $u(\triangle)$ by Lemma \ref{lem:isomtri}.
Denote by $N$ the number of triangles in $\tau$.
Then, $\llangle\DD\rrangle_u$ decomposes into $N$ convex sets which are mapped isometrically by the induced map $\bar u$; 
in particular, $\llangle\DD\rrangle_u$ belongs to $\mathcal{S}_N$.  
...
Since $\mathcal{S}_N$ is compact, we can pass to a partial limit of $\llangle\DD\rrangle_{u_n}$ as $n\to\infty$.
...
By Lemma \ref{lem:proj-continuous}, the projection $\pi_k\:\DD\to \llangle\DD\rrangle_{u_k}$ is continuous
...
Since the decompositions of $\llangle\DD\rrangle_{u_n}$ into convex subset of $W$ are nested for varying $n$, they
induce a unique such limit decomposition of $Q_\eps$.
Moreover, $w_\eps$ restricts to an isometry on each 
of the convex sets occurring in this decomposition.
...
in contradiction to $w_\eps$ being saddle.
...
Now, assume that the boundary of $\llangle\DD\rrangle_s$ is rectifiable.
...
Further note that by construction, there is a map $s'$ arbitrary close to $s$ 
...
Note that there is a map $p\:\partial \|\DD\|_s\to W$ which satisfy the assumptions in the finite-whole lemma (\ref{lem:finite-whole});
that is given a finite set $F\subset \DD$...
Moreover, there is a short map $q\:\mathcal{W}\to Y$ such that $q\circ p=\id_{\partial \|\DD\|_s}$.
To construct $p$ and $q$ choose a dense sequence of points $\{x_1,x_2,\dots\}$ in $\partial \|\DD\|_s$,
fix a finite set $F\subset \|\DD\|_s$,
pass to the $\omega$limit maps for the maps provided by the key lemma (\ref{lem:key}) for the sequence of sets 
$F_n=F\cup\{x_1,\dots, x_n\}$ and project the resulting map to the convex subset $Y$ in $Y^\omega$.
%%%???NOT QUITE BUT CLOSE???

Applying the finite-whole lemma (\ref{lem:finite-whole}) we obtain a short (and therefore length-preserving) map from $\|D\|_s$ to $W$. ???
%%%%???
