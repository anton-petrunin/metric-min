...
The image $s(\DD)\subset X$ with induced intrinsic metric will be denoted as $\|D\|_s$.%
...
and minimal surfaces. Indeed our result combined with the smoothability of $\CAT[0]$-surfaces %???ref???
...
$f(B_\delta(x))\subset B_\eps(f(x))$. If $x_k\in B_\delta(x)$, then there is a path in $X$ connecting $x_k$
...
\parbf{Intrinsic metric determines metric minimizing discs.}
...
\parbf{Metric minimizing discs below given continuous ones.}
...
$u$ is saddle. By Lemma \ref{lem:CL}, every point $x$ in $\DD$ and every $\eps>0$
