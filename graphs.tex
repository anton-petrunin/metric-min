...
\parit{Proof.}
On each edge of $\Gamma$, fix a parametrization by $[0,1]$.
A map $h\:\Gamma\to Y$ will be called \emph{straight} if it
sends each edge of $\Gamma$ to a geodesic path in $Y$.

Without loss of generality, we may assume that $f$ is straight.

Let $\mathcal{O}$ be a maximal well-ordered subset of straight maps with respect to ``$\succcurlyeq\rel A$'' with $f$ as the maximal element . 
We need to show that $\mathcal{O}$ has a minimal element.
...
\mathcal{F}
=
\set{Q\subset\mathcal{O}}{\mathcal{O}-Q\text{ has a lower bound}}.
$$
is nonprincipal filter.
Complete $\mathcal{F}$ to an ultrafilter $\omega$ on $\mathcal{O}$. 
...
$f$ and, therefore, all maps in $\mathcal{O}$ are Lipschitz continuous. 
